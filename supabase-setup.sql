...
-- Creates all required tables in the PUBLIC schema (explicit public. to avoid schema conflicts)
...
-- Grocery Payments
CREATE TABLE IF NOT EXISTS public.grocery_payments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id TEXT NOT NULL REFERENCES public.businesses(id),
...
CREATE TABLE IF NOT EXISTS public.external_asset_owners (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  business_id TEXT NOT NULL REFERENCES public.businesses(id),
...
-- Fleet Vehicles (avoids conflict with Hapyjo public.vehicles)
...
CREATE TABLE IF NOT EXISTS public.rental_contracts (
  ...
  external_owner_id UUID REFERENCES public.external_asset_owners(id),
...
-- Notifications (for real-time feed)
...
-- Grant access to public schema tables (only affects public schema)
